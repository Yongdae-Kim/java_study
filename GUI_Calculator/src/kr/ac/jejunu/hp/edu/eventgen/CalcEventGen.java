...
import kr.ac.jejunu.hp.edu.event.CleanKeyEvent;
import kr.ac.jejunu.hp.edu.event.DotsKeyEvent;
...
public class CalcEventGen {
	...
	public void generateEvents(JTextField jtf, String btnName) {
		...
		new DotsKeyEvent().execute(jtf, btnName);

		new BackspaceKeyEvent().execute(jtf, btnName);
		new CleanKeyEvent().execute(jtf, btnName);
